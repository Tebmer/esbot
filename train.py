...
parser.add_argument('--save_model', type=boolean_string, default=True, help='save model. if false, we do not save model for saving space.')
# do normal parsing
...
if args.local_rank == -1 or get_rank() == 0:
    ...
    print('epoch,global_step,step,tmp_loss,tmp_ppl,tmp_strat_loss,mean_loss,mean_ppl,n_token_real,'
          'n_token_total,epoch_time', file=train_logger)
    print('epoch,global_step,step,freq_loss,freq_ppl,eval_strat_loss', file=eval_logger)
...
while True:
    ...
    tr_loss: trace loss, i.e. accumulated with batch step.
    mean_loss: mean_loss = tr_loss / nb_tr_steps, i.e. mean accumulated loss.
    """
    ...
    for batch in train_dataloader:
        ...
        ppl = outputs.pop('ppl')
        strat_loss = outputs.pop('strat_loss', None)
        
        if strat_loss is not None:
            total_loss = loss + strat_loss
        ...
        if n_gpu > 1:
            ...
            ppl = ppl.mean()
        total_loss = total_loss / (args.train_batch_size * args.gradient_accumulation_steps / input_ids.shape[0])
        if args.fp16:
            with amp.scale_loss(loss, optimizer) as scaled_loss:
                scaled_loss.backward()
        else:
        ...
        if strat_loss is not None:
            tmp_strat_loss = float(strat_loss.item()) * (args.train_batch_size * args.gradient_accumulation_steps / input_ids.shape[0])
        else:
            tmp_strat_loss = float(0.0)
        tr_loss += tmp_loss
        ...
        if step % args.gradient_accumulation_steps == 0:
            ...
            if args.local_rank == -1 or get_rank() == 0:
                ...
                if pbar is not None:
                    ...
                    pbar_str += f"lm_loss: {mean_loss:.2f} strat_loss: {tmp_strat_loss:.2f} ppl: {mean_ppl:.2f} epoch: {epoch}"
                ...
                print(f'{epoch+1},{global_step+1},{step+1},{tmp_loss},{tmp_ppl},{tmp_strat_loss},{mean_loss},{mean_ppl},'
            ...
            if args.num_epochs is None and global_step % args.valid_step == 0:# and epoch > 0:
                if args.local_rank == -1 or get_rank() == 0:
                    ...
                    eval_loss, eval_ppl, eval_strat_loss, eval_samples, *_ = eval_model_loss(
                        model=model,
                    ...
                    print(f'{epoch+1},{global_step+1},{step+1},{eval_loss},{eval_ppl},{eval_strat_loss}', file=eval_logger)
    ...
    if args.num_epochs is not None:
        if args.local_rank == -1 or get_rank() == 0:
            ...
            print(f'{epoch},{global_step+1},{step+1},{eval_loss},{eval_ppl},{eval_strat_loss}', file=eval_logger)
            logger.info('current learning rate: '
                        + str(optimizer.param_groups[0]['lr']))
            model.train()
