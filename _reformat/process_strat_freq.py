...
def process_data(d):
    ...
    for uttr in d:
        ...
        if role == 'seeker':
            ...
            strategy.append(uttr['annotation']['strategy'])
    res = {
...
with mp.Pool(processes=mp.cpu_count()) as pool:
    for e, s in pool.imap(process_data, tqdm.tqdm(original, total=len(original))):
        data.append(e)
        all_strategy.extend(s)
        
strategies_cnt = Counter([e for e in all_strategy])
strategies_freq = {strat: cnt / len(all_strategy) for strat, cnt in strategies_cnt.items()}

print(strategies_cnt)
print(strategies_freq)
