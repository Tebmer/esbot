...
from src.transformers import (AutoTokenizer, AutoModel, AutoConfig)
...
def build_model(only_toker=False, checkpoint=None, local_rank=-1, **kwargs):    
    # Load the customed config json file.
    if checkpoint:
        exp_dir = '/'.join(checkpoint.split('/')[:-1])
        if not os.path.exists(f'{exp_dir}/custom_config.json'):
            raise ValueError
        with open(f'{exp_dir}/custom_config.json', 'r', encoding='utf-8') as f:
            config = json.load(f)
    else:
    ...
    if 'strat_seq_model_config' in config:
        strat_seq_model_config = config['strat_seq_model_config']
        strat_seq_model_config.update({
            'num_hiddens': model.config.d_model,
            'input_size': model.config.d_model,
        })
        model.strat_seq_model = StratSeqModel(strat_seq_model_config)
