...
class BlenderbotSmallDecoderLayer(nn.Module):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        self.encoder_atten_strategy = BlenderbotSmallAttention(
            self.embed_dim,
            config.decoder_attention_heads,
            dropout=config.attention_dropout,
            is_decoder=True,
    ...
    def forward(
        ...
        encoder_strategy_embeds: Optional[torch.Tensor] = None
    ):
        """
        Args:
        ...
        if encoder_hidden_states is not None:
            ...
            # Strategy Cross-Attention Block
            if encoder_strategy_embeds is not None: 
                hidden_states_strategy, _, _ = self.encoder_atten_strategy(
                    hidden_states=hidden_states,
                    key_value_states=encoder_strategy_embeds,
                )
                hidden_states_strategy = nn.functional.dropout(hidden_states_strategy, p=self.dropout, training=self.training)
                hidden_states = hidden_states + hidden_states_strategy
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    encoder_strategy_embeds=encoder_strategy_embeds
                )
            hidden_states = layer_outputs[0]
