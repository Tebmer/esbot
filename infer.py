...
parser.add_argument ("--golden_strategy", action="store_true", 
                     help="when generating, use the golden strategy, i.e. the Oracle model mentioned in the paper.")
...
with open(f'./CONFIG/{args.config_name}.json', 'r', encoding='utf-8') as f:
    config = json.load(f)
    strategy_types = config.get('strategy_types', None)
...
if args.golden_strategy:
    logger.info("Oracle generating mode.")
else:
    logger.info("Joint generating mode.")
...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    if not args.only_encode and not args.only_generate:
        ...
        infer_loss, _, _, infer_samples, pointwise_loss, pointwise_sample = eval_model_loss(
            model=model,
        ...
        logger.info(f"Perplexity: {metric_res['perplexity']}")
        ptr = 0
    ...
    # 读取test数据集, 然后分batch进行`generate`操作得到生成结果.
    # 对于每一个样本, 将其结果存入到tmp_res_to_append中, 包括一下字段: 
    #   sample_id, post, response(真实的), generation(预测的), token_num, ppl, loss
    # 注意: 结果都是 句子级 的.
    ...
    for batch, posts, references, sample_ids in infer_dataloader:
        ...
        # 1) Get generation result
        encoded_info, generations = model.generate(**batch)
        
        # 2) Get `other_res`, namely the result of strategy classification, and save them into `other_res`.
        # Note: The `other_res` is used to calculate the accuracy of all samples. 
        ...
        if 'other_res' in batch:
            ...
            if add_acc:
                ...
                # Notice: 
                # other_res['acc_map'] = {'cls_strat_id': 'pred_strat_id'}
                # So, k='cls_strat_id' (i.e. labels) and v='pred_strat_id' (i.e. prediction)
        ...
        # Process each sample in batch data, including generations and other_res (i.e. strategy classification accuracy)
        for idx in range(len(sample_ids)):
            ...
            # `other_res_to_append` store the strategy classification result for *each sample*.
            other_res_to_append = {}
    ...
    infer_mode = "oracle" if args.golden_strategy else "joint"
    if not args.only_encode:
        save_dir = f'{checkpoint_dir_path}/mode_{infer_mode}_res_{checkpoint_name}_{infer_input_file_name}_k.{args.top_k}' \
    ...
    # Evaluation of accuracy of strategy classification
    if not args.only_generate:
        if 'acc' in other_res:
    ...
    # Evaluation of BLEU, ROUGE, and other NLG metrics.
