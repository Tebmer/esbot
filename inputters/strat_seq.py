...
class InputFeatures(object):
    def __init__(
        ...
        self.strat_seq = strat_seq


def featurize(
    bos, eos,
    context, max_input_length,
    response, max_decoder_input_length, strat_id, strat_seq
):
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    start_num = len(strategy_types)
    idx2strats = strategy_types
    strat2idxs = {strat: idx for idx, strat in enumerate(idx2strats)}
    begin_strat_id = start_num
    # Add `begin_strat_id` for predict the first one strategy.
    strat_seq = [begin_strat_id]
    ...
    for i in range(len(dialog)):
        ...
        if dialog[i]['speaker'] == 'sys':
            strategy = '[' + dialog[i]['strategy'] + ']'
            strat_id = strat2idxs[strategy]
            strat_seq.append(strat_id)
        
        if i > 0 and dialog[i]['speaker'] == 'sys':
            res = {
                'context': context.copy(),
                'response': text,
                'strat_id': strat_id,   # Target strategy sequence
                'strat_seq': strat_seq.copy()[: -1] # History strategy sequence
            }
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        if not infer:
            ...
            # f.decoder_input_ids[0] is `bos` token
            decoder_input_ids = torch.tensor([[f.decoder_input_ids[0]] for f in features], dtype=torch.long)
            labels = None
        

        
        strat_id = torch.tensor([f.strat_id for f in features], dtype=torch.long)
        # strat_seq = torch.tensor([f.strat_seq for f in features], dtype=torch.long)
        strat_seq = pad_sequence([torch.tensor(f.strat_seq, dtype=torch.long) for f in features], 
                     batch_first=True, padding_value=0)
        strat_seq_len = torch.tensor([len(f.strat_seq) for f in features], dtype=torch.long)
...
# for inference
def prepare_infer_batch(features, toker, interact=None, golden_strategy=True):
    ...
    if interact is None and golden_strategy:
        other_res['cls_strat_id'] = res['strat_seq_input'].get('strat_id')
    else:
        other_res['cls_strat_id'] = res['strat_seq_input'].pop('strat_id')
...
def get_infer_batch(infer_input_file, toker, **kwargs):
    ...
    golden_strategy = kwargs.get('golden_strategy')
    ...
    for sample_id, line in tqdm.tqdm(enumerate(reader), total=len(reader), desc=f"inferring"):
        ...
        for i in range(len(inputs)):
            ...
            if len(sample_ids) == infer_batch_size:
                yield prepare_infer_batch(features, toker, golden_strategy=golden_strategy), posts, references, sample_ids
                features = []
                sample_ids = []
                posts = []
                references = []
